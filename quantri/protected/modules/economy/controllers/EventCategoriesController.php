...
class EventCategoriesController extends BackController {
    ...
    public function actionCreate($id) {
        ...
        $this->breadcrumbs = array(
            Yii::t('event', 'event_category') => Yii::app()->createUrl('/economy/eventCategories/'),
            Yii::t('event', 'event_category_create') => Yii::app()->createUrl('/economy/eventCategories/create', array('id' => $id)),
        );
        ...
        if ($id != 0) {
            $parent_model = EventCategories::model()->findByPk($id);
        ...
        $post = Yii::app()->request->getPost('EventCategories');
        ...
        if (Yii::app()->request->isPostRequest && $post) {
            ...
            if ($id == 0) {
                ...
                $model2 = EventCategories::model()->findByPk($id);
                if ($model2) {
            ...
            if ($model->save()) {
                ...
                $this->redirect(Yii::app()->createUrl("/economy/eventCategories"));
            }
        }
    ...
    public function actionUpdate($id) {
        ...
        $this->breadcrumbs = array(
            Yii::t('event', 'event_category') => Yii::app()->createUrl('/economy/eventCategories/'),
            Yii::t('event', 'event_category_update') => Yii::app()->createUrl('/economy/eventCategories/update', array('id' => $id)),
        );
        ...
        $cat = Yii::app()->request->getPost('EventCategories');
    ...
    public function actionAdmin() {
        $model = new EventCategories('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['EventCategories']))
            $model->attributes = $_GET['EventCategories'];
    ...
    public function loadModel($id) {
        ...
        $EventCategories->setTranslate(false);
        //
        $OldModel = $EventCategories->findByPk($id);
        //
        if ($OldModel === NULL)
        ...
        if (ClaSite::getLanguageTranslate()) {
            $EventCategories->setTranslate(true);
            $model = $EventCategories->findByPk($id);
            if (!$model) {
                $model = new EventCategories();
                $model->id = $id;
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'event-categories-form') {
    ...
    public function actionUploadfile() {
        if (isset($_FILES['file'])) {
            ...
            $up->setPath(array($this->site_id, 'eventcategory', 'ava'));
            $up->uploadImage();
            $return = array();
