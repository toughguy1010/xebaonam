...
class TranslateOrderController extends BackController
{
    ...
    {
        if (isset($_POST['TranslateOrder'])) {
            $model = $this->loadModel($id);
            $model->user_id = $_POST['TranslateOrder']['user_id'];
            if ($model->save()) {
    ...
    public function actionUpdate($id)
    {
        ...
        if (isset($_POST['TranslateOrder'])) {
            $model->attributes = $_POST['TranslateOrder'];
            if ($model->save()) {
                $this->redirect(Yii::app()->createUrl('economy/translateOrder'));
            }
        }
        //
        //
    ...
    function actionEdit($id)
    {
        ...
        if ($model->order_status == TranslateOrder::ORDER_COMPLETE)
            $this->sendResponse(404);
        //
        if ($model->viewed == TranslateOrder::ORDER_NOTVIEWED)
            $model->viewed = TranslateOrder::ORDER_VIEWED;
        //
        $products = OrderProducts::getProductsDetailInOrder($id);
        $paymentmethod = TranslateOrder::getPaymentMethodInfo($model->payment_method);
        $transportmethod = TranslateOrder::getTransportMethodInfo($model->transport_method);
        $bonusConfig = BonusConfig::checkBonusConfig();
        $orderInfo = $model->attributes;
        $oldtransportmethod = $model->transport_method;
        //
        $error = $model->getErrors();
        if (isset($_POST['TranslateOrder']) && $model->order_status != TranslateOrder::ORDER_COMPLETE) {
            $model->order_status = $_POST['TranslateOrder']['order_status'];
            $model->payment_status = $_POST['TranslateOrder']['payment_status'];
            $model->transport_status = $_POST['TranslateOrder']['transport_status'];
            $model->transport_freight = $_POST['TranslateOrder']['transport_freight'];
            $model->discount_percent = $_POST['TranslateOrder']['discount_percent'];
            if ($model->validate()) {
                $model->discount_percent = $_POST['TranslateOrder']['discount_percent'];
...
//breadcrumbs
        $this->breadcrumbs = array(
            Yii::t('shoppingcatr', Yii::t('shoppingcart', 'order_translate_manager')) => Yii::app()->createUrl('/economy/translateOrder')
        );
        $model = new TranslateOrder('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['TranslateOrder'])) {
            $model->attributes = $_GET['TranslateOrder'];
//            $model->from_date = $_GET['TranslateOrder']['from_date'];
//            $model->to_date = $_GET['TranslateOrder']['to_date'];
        }
...
//
     ...
     * @return TranslateOrder the loaded model
     ...
     * @param TranslateOrder $model the model to be validated
     */
    protected
    function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'TranslateOrder-form') {
