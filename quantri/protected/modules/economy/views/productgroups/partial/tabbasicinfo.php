...
<div class=" form-group no-margin-left">
    ...
    <div class="controls col-sm-10">
        ...
        <div id="productgroupsavatar" style="display: block; margin-top: 0px;">
            <div id="productgroupsavatar_img"
            ...
            <div id="productgroupsavatar_form" style="display: inline-block;">
...
<script type="text/javascript">
    jQuery(function ($) {
        jQuery('#productgroupsavatar_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/economy/productgroups/uploadfile"); ?>',
            name: 'file',
            onSubmit: function () {
            },
            onComplete: function (result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#ProductGroups_avatar').val(obj.data.avatar);
                        if (jQuery('#productgroupsavatar_img img').attr('src')) {
                            jQuery('#productgroupsavatar_img img').attr('src', obj.data.realurl);
                        } else {
                            jQuery('#productgroupsavatar_img').append('<img src="' + obj.data.realurl + '" />');
                        }
                        jQuery('#productgroupsavatar_img').css({"margin-right": "10px"});
                    }
                }
            }
        });
...
<script type="text/javascript">
    function removeAvatar(id) {
        if (confirm("Are you sure delete avatar?")) {
            $.getJSON(
                '<?php echo Yii::app()->createUrl('economy/productgroups/deleteAvatar') ?>',
                {id: id},
                function (data) {
                    if (data.code == 200) {
                        $('#productgroupsavatar_img img').remove();
                    }
                }
            );
        }
