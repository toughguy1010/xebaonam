        ...
        <div class="widget-main">
            <div class="row">
                <div class="col-xs-12 no-padding">
                    ...
                    <div class="control-group form-group">
                        <?php echo $form->labelEx($model, 'skill', array('class' => 'col-sm-2 control-label no-padding-left')); ?>
                        <div class="controls col-sm-8" id="skill-list">
                            <?php if (isset($model->skill)) {
                                $arySkill = json_decode($model->skill,true);
                                $countSkill = count($arySkill);
                                foreach ($arySkill as $key => $value) {
                                    ?>
                                    <div class="control-group form-group" id="dev-<?=$key?>">
                                        <input class="col-sm-5" placeholder="Tên kỹ năng" type="text"
                                               name="skill[<?= $key ?>][name]" value="<?= $value['name'] ?>">
                                        <input class="col-sm-5" placeholder="Trình dộ" type="text"
                                               name="skill[<?= $key ?>][value]" value="<?= $value['value'] ?>">
                                        <button type="button" class="col-sm-2 btn btn-xs btn-danger" data-number=""
                                                onclick="delDiv(<?= $key ?>)">Del
                                        </button>
                                    </div>
                                    <?php
                                }
                        ...
                        <button class=" col-sm-2 btn btn-sm btn-info" type="button" onclick="addLink(this)"
                                data-number="<?=(isset($countSkill)) ? $countSkill : 0 ?>">Add
                        </button>
                    </div>
                    ...
                    <script>
                        function addLink(ev) {
                            var number = $(ev).attr('data-number');
                            $('#skill-list').append('<div class="control-group form-group" id="dev-' + number + '"><input class="col-sm-5" placeholder="Tên kỹ năng" type="text" name="skill[' + number + '][name]"><input onclick="removeDisabled()" class="col-sm-5" placeholder="Trình dộ" type="text" name="skill[' + number + '][value]"><button type="button" class="col-sm-2 btn btn-xs btn-danger" data-number="' + number + '" onclick="delDiv(' + number + ')">Del</button></div>');
                            $(ev).attr('data-number', ++number);
                        }
                        function delDiv(ev) {
                            $('#dev-' + ev).remove();
                        }
