...
<div class="widget widget-box">
    <div class="widget-header"><h4>
            <?php echo Yii::app()->controller->action->id != "editcat" ? Yii::t('rent', 'category_rent_create') : Yii::t('rent', 'category_rent_update'); ?>
        </h4></div>
                    ...
                    <div class="control-group form-group">
                        ...
                        <div class="controls col-sm-10">
                            ...
                            <div id="categoryicon" style="display: block; margin-top: 0px;">
                                ...
                                <div style="display: inline-block;">
                                    <?php if ($model->icon_path && $model->icon_name) { ?>
                                        <input class="btn btn-sm" type="submit" name="remove_icon" value="Xóa ảnh">
                                    <?php } ?>
                                </div>
                            </div>
...
<script type="text/javascript">
            ...
            'success': function (res) {
                if (res.code != "200") {
                    ...
                    jQuery.ajax({
                        'type': 'POST',
                        'dataType': 'JSON',
                        'url': '<?php echo Yii::app()->createUrl('suggest/suggest/category', array('type' => ClaCategory::CATEGORY_RENT)); ?>',
                        'success': function (res) {
                            if (res.code == '200') {
                                if (res.html)
    ...
    jQuery(function ($) {
        jQuery('#categoryavatar_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/economy/rentcategory/uploadfile"); ?>',
            name: 'file',
            onSubmit: function () {
            },
            onComplete: function (result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#RentCategories_avatar').val(obj.data.avatar);
        ...
        jQuery('#categoryicon_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/economy/rentcategory/uploadfileicon"); ?>',
            name: 'file',
            onSubmit: function () {
            },
            onComplete: function (result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#RentCategories_icon').val(obj.data.icon);
        ...
        jQuery('#categorysize_chart_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/economy/rentcategory/uploadfileSizechart"); ?>',
            name: 'file',
            onSubmit: function () {
            },
            onComplete: function (result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#RentCategories_size_chart').val(obj.data.size_chart);
    ...
    function removeImage(type_image) {
        var url_request = "<?php echo Yii::app()->createUrl('economy/rentcategory/removeImageCat') ?>";
        jQuery.getJSON(
            url_request,
            {type: type_image},
            function (data) {
    ...
    jQuery(document).ready(function () {
        $('#ck-check').on("click", function () {
            if (this.checked) {
                CKEDITOR.replace("RentCategories_cat_description", {
                    height: 400,
                    language: '<?php echo Yii::app()->language ?>'
                });
            } else {
                var a = CKEDITOR.instances['RentCategories_cat_description'];
                if (a) {
                    a.destroy(true);
                }
