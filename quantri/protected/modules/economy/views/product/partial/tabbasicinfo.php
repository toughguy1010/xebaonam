...
<div class="form-group no-margin-left">
    <?php echo $form->labelEx($model, 'barcode', array('class' => 'col-sm-2 control-label no-padding-left')); ?>
    <div class="controls col-sm-10">
        <?php echo $form->textField($model, 'barcode', array('class' => 'span12 col-sm-12')); ?>
...
<div class="form-group no-margin-left">
    ...
    <div class="controls col-sm-10">
        <?php echo $form->textField($model, 'color', array('class' => 'span12 col-sm-12')); ?>
...
<div class="controls col-sm-10" id="producticon">
    <?php if (!$model->isNewRecord && $model->icon_name && $model->icon_path) { ?>
        ...
        <div style="margin-top: 15px;">
            <button id="remove_icon" type="button" onclick="removeIcon(<?= $model->id ?>)"
...
?>
<?php
if ($siteinfo['site_skin'] == 'w3ni6881') {
    ?>
    <div class="form-group no-margin-left">
        <label class="col-sm-2 control-label no-padding-left required" for="Product_manufacturer_id">Danh mục sản phẩm
            <span class="required">*</span> </label>
        <div class="controls col-sm-10">
            <div class="input-group">
                <?php
                $categories = ProductCategories::getCategoryByParent(0);
                ?>
                <?php
                $categoryTrack = trim($model->category_track);
                $categoryTrackArray = [];
                if (isset($categoryTrack) && $categoryTrack) {
                    $categoryTrackArray = explode(' ', $categoryTrack);
                }

                ?>
                <div class="wrapper-category">
                    <div class="fl div-menucategory-0" currentid="0">
                        <h6>&nbsp;Cha&nbsp;</h6>
                        <select class="menucategory" multiple="multiple">
                            <?php foreach ($categories as $cat) { ?>
                                <option <?= in_array($cat['cat_id'], $categoryTrackArray) ? 'selected' : '' ?>
                    ...
                    <?php
                    if (isset($categoryTrackArray) && $categoryTrackArray) {
                        foreach ($categoryTrackArray as $cat_id) {
                            $dataChildren = ProductCategories::getCategoryByParent($cat_id);
                            if (isset($dataChildren) && $dataChildren) {
                                $modelManu = ProductCategories::model()->findByPk($cat_id);
                                ?>
                                <div class="fl div-menucategory-<?= $cat_id ?> parent-id-<?= $modelManu['cat_parent'] ?>"
                                     currentid="<?= $cat_id ?>">
                                    <h6>&nbsp;<?= $modelManu['cat_name'] ?>&nbsp;</h6>
                                    <select class="menucategory" multiple="multiple">
                                        <?php foreach ($dataChildren as $cat) { ?>
                                            <option <?= in_array($cat['cat_id'], $categoryTrackArray) ? 'selected="selected"' : '' ?>
                                                    value="<?= $cat['cat_id'] ?>"><?= $cat['cat_name'] ?>⇒
                                            </option>
                                        <?php } ?>
                                    </select>
                ...
                <input type="hidden" id="product_category_id" name="Product[product_category_id]"
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $(".wrapper-category").on("click", ".menucategory option", function () {
                var cat_id = $(this).val();
                $('#product_category_id').val(cat_id);
                ...
                for (var i in diff) {
                    $('.div-menucategory-' + diff[i]).remove();
                    $('.parent-id-' + diff[i]).remove();
                }
                if ($(this).is(':selected')) {
                    if (!$('.div-menucategory-' + cat_id).length) {
                        $.getJSON(
                            '<?= Yii::app()->createUrl("/economy/product/getCategoryChildren") ?>',
                            {cat_id: cat_id},
                            function (data) {
                                $('.wrapper-category').append(data.html);
                            }
                        );
                    }
                } else {
                    $('.div-menucategory-' + cat_id).remove();
            ...
            <div class="box-add-cat">
                        ...
                        if ($rels) {
                            ...
                            foreach ($rels as $key => $value) {
                                ...
                                <div class="item-cat">
                                    ...
                                    ?>
                                    <span class="delete-cat col-md-1"
                                          data-id="<?= $rel->id ?>">x</span>
                                </div>
                ...
                <script type="text/javascript">
                    $('.add-select-cat').click(function () {
                        $('#box-append-cat').append('<div class="item-cat"><select class="col-md-11" name="new_rel_cal[]">' + $('#Product_product_category_id').html() + ' </select><span class="delete-cat col-md-1">x</span></div>');
                    });
...
<?php
$manufacturer_all = ManufacturerCategories::getManufacturerAll();
if (isset($manufacturer_all) && $manufacturer_all) {
    ?>
    <div class="form-group no-margin-left">
...
<div class="form-group no-margin-left">
    <?php echo $form->labelEx($model, 'run_discount', array('class' => 'col-sm-2 control-label no-padding-left')); ?>
    <div class="controls col-sm-10">
        <?php echo $form->checkBox($model, 'run_discount'); ?>
        <?php echo $form->error($model, 'run_discount'); ?>
    </div>
</div>
<div class="form-group no-margin-left">
    <?php echo $form->labelEx($model, 'expire_discount', array('class' => 'col-sm-2 control-label no-padding-left')); ?>
    <div class="controls col-sm-10">
        <?php
        $this->widget('common.extensions.EJuiDateTimePicker.EJuiDateTimePicker', array(
            'model' => $model, //Model object
            'name' => 'Product[expire_discount]', //attribute name
            'mode' => 'datetime', //use "time","date" or "datetime" (default)
            'value' => ((int)$model->expire_discount > 0) ? date('d-m-Y H:i:s', (int)$model->expire_discount) : date('d-m-Y H:i:s'),
            'language' => 'vi',
            'options' => array(
                'showSecond' => true,
                'dateFormat' => 'dd-mm-yy',
        ...
        ?>
        <?php echo $form->error($model, 'expire_discount'); ?>
    </div>
</div>
...
<div class="form-group no-margin-left">
    <?php echo $form->labelEx($model, 'isselling', array('class' => 'col-sm-2 control-label no-padding-left')); ?>
    <div class="controls col-sm-10">
        <?php echo $form->checkBox($model, 'isselling'); ?>
        <?php echo $form->error($model, 'isselling'); ?>
    </div>
</div>
<div class="form-group no-margin-left">
...
<div class="form-group no-margin-left">
    <?php echo $form->labelEx($model, 'unmarked', array('class' => 'col-sm-2 control-label no-padding-left')); ?>
    <div class="controls col-sm-10">
        <?php echo $form->checkBox($model, 'unmarked'); ?>
        <?php echo $form->error($model, 'unmarked'); ?>
    </div>
</div>
...
<script type="text/javascript">
    function removeIcon(id) {
        if (confirm("Are you sure delete avatar?")) {
            $.getJSON(
                '<?php echo Yii::app()->createUrl('economy/product/deleteIcon') ?>',
                {id: id},
                function (data) {
                    if (data.code == 200) {
                        $('#producticon img').remove();
                        $('#remove_icon').remove();
                    }
                }
            );
