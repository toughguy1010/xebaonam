...
<div class="widget widget-box">
    <div class="widget-header">
        <h4>
            <?php echo Yii::app()->controller->action->id != "update" ? Yii::t('rent', 'destinations') : Yii::t('rent', 'update_destinations'); ?>
        </h4>
    </div>
                ...
                <div class="col-xs-12 no-padding">
                    ...
                    <div class="control-group form-group">
                        ...
                        <div class="controls col-sm-10">
                            ...
                            <div id="destinationsavatar" style="display: block; margin-top: 0px;">
                                <div id="destinationsavatar_img"
                                ...
                                <div id="destinationsavatar_form" style="display: inline-block;">
                    ...
                    <div class="control-group form-group buttons">
                        <?php echo CHtml::submitButton($model->isNewRecord ? Yii::t('rent', 'destinations') : Yii::t('rent', 'update_destinations'), array('class' => 'btn btn-primary', 'id' => 'btnAddCate')); ?>
                    </div>
                    <?php
                    $this->endWidget();
                    ?>
...
<script type="text/javascript">
    jQuery(document).on('change', '#Destinations_province_id', function () {
        jQuery.ajax({
            url: '<?php echo Yii::app()->createUrl('/suggest/suggest/getdistrict') ?>',
            data: 'pid=' + jQuery('#Destinations_province_id').val(),
            dataType: 'JSON',
            beforeSend: function () {
                w3ShowLoading(jQuery('#Destinations_province_id'), 'right', 20, 0);
            },
            success: function (res) {
                if (res.code == 200) {
                    jQuery('#Destinations_district_id').html(res.html);
                }
                w3HideLoading();
                getWard();
            },
    ...
    jQuery(document).on('change', '#Destinations_district_id', function () {
        getWard();
    });
    ...
    function getWard() {
        jQuery.ajax({
            ...
            data: 'did=' + jQuery('#Destinations_district_id').val(),
            dataType: 'JSON',
            beforeSend: function () {
                w3ShowLoading(jQuery('#Destinations_district_id'), 'right', 20, 0);
            },
            success: function (res) {
                if (res.code == 200) {
                    jQuery('#Destinations_ward_id').html(res.html);
                }
                w3HideLoading();
            },
            error: function () {
...
<script type="text/javascript">
    jQuery(function ($) {
        jQuery('#destinationsavatar_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/economy/destinations/uploadfile"); ?>',
            name: 'file',
            onSubmit: function () {
            },
            onComplete: function (result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#Destinations_avatar').val(obj.data.avatar);
                        if (jQuery('#destinationsavatar_img img').attr('src')) {
                            jQuery('#destinationsvatar_img img').attr('src', obj.data.realurl);
                        } else {
                            jQuery('#destinationsavatar_img').append('<img src="' + obj.data.realurl + '" />');
                        }
                        jQuery('#destinationsavatar_img').css({"margin-right": "10px"});
                    }
                }
            }
        });
