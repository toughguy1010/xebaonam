...
<div class="widget widget-box">
    <div class="widget-header"><h4>
            <?php echo Yii::app()->controller->action->id != "editcat" ? Yii::t('category', 'category_Jobs_create') : Yii::t('category', 'category_Jobs_update'); ?>
        </h4></div>
                ...
                <div class="col-xs-12 no-padding">
                    ...
                    <div class="control-group form-group">
                        ...
                        <div class="controls col-sm-10">
                            ...
                            <label for="ck-check" style="font-size: 11px;color: blue;font-weight: bold">Sử dụng trình
                                soạn thảo</label>
                        ...
                        <div class="controls col-sm-10">
                            ...
                            <div id="categoryavatar" style="display: block; margin-top: 0px;">
                                ...
                                <div style="display: inline-block;">
                                    <?php if ($model->image_path && $model->image_name) { ?>
                                        <button id="btn-rm" type="button" onclick="removeAva(<?= $model->cat_id ?>)"
                            ...
                            <div id="categorycover" style="display: block; margin-top: 0px;">
                                <div id="categorycover_img"
                                    ...
                                    <?php if ($model->cover_path && $model->cover_name) { ?>
                                        <img src="<?php echo ClaHost::getImageHost() . $model->cover_path . 's100_100/' . $model->cover_name; ?>"
                                             style="width: 100%;"/>
                                    <?php } ?>
                                </div>
                    ...
                    <?php if (ClaUser::isSupperAdmin()) { ?>
                        <div class="control-group form-group">
                            ...
                            <div class="controls col-sm-10">
                                <?php echo $form->textField($model, 'limit', array('class' => 'span10 col-sm-12')); ?>
...
<script type="text/javascript">
    function removeAva(id) {
        if (confirm("Are you sure delete icon?")) {
            $.getJSON(
                '<?php echo Yii::app()->createUrl('work/Jobscategory/DeleteAvatar') ?>',
                {id: id},
                function (data) {
                    if (data.code == 200) {
    ...
    function removeIco(id) {
        if (confirm("Are you sure delete icon?")) {
            $.getJSON(
                '<?php echo Yii::app()->createUrl('work/Jobscategory/DeleteIco') ?>',
                {id: id},
                function (data) {
                    if (data.code == 200) {
                        $('#categorycover_img img').remove();
                        $('#btn-rm-icon').remove();
                    }
                }
            );
    ...
    jQuery(function ($) {
        $('#ck-check').on("click", function () {
            if (this.checked) {
                CKEDITOR.replace("JobsCategories_cat_description", {
                    height: 300,
                    language: '<?php echo Yii::app()->language ?>'
                });
            } else {
                var a = CKEDITOR.instances['JobsCategories_cat_description'];
                if (a) {
                    a.destroy(true);
                }
                ...
                'success': function (res) {
                    if (res.code != "200") {
                        ...
                        jQuery.ajax({
                            'type': 'POST',
                            'dataType': 'JSON',
                            'url': '<?php echo Yii::app()->createUrl('suggest/suggest/category', array('type' => ClaCategory::CATEGORY_Jobs)); ?>',
                            'success': function (res) {
                                if (res.code == '200') {
                                    if (res.html)
                                        jQuery('#Jobs_Jobs_category_id').html(res.html);
        ...
        jQuery('#categoryavatar_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/work/Jobscategory/uploadfile"); ?>',
            name: 'file',
            onSubmit: function () {
            },
            onComplete: function (result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#JobsCategories_avatar').val(obj.data.avatar);
        ...
        jQuery('#categorycover_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/work/Jobscategory/uploadCoverfile"); ?>',
            name: 'file',
            onSubmit: function () {
            },
            onComplete: function (result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#JobsCategories_cover').val(obj.data.cover);
