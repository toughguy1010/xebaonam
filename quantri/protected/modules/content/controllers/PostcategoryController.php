...
class PostcategoryController extends BackController {
    ...
    public function actionIndex() {
        ...
        $this->breadcrumbs = array(
            Yii::t('post', 'post_category') => Yii::app()->createUrl('/content/postcategory'),
        );
        //
        $model = new PostCategories();
        $cid = Yii::app()->request->getParam('cid');
        if ($cid) {
            $category = PostCategories::model()->findByPk($cid);
            if (!$category || $category->site_id != $this->site_id)
                return false;
            //breadcrumbs
            $this->breadcrumbs = array(
                $category->cat_name => Yii::app()->createUrl('/content/postcategory', array('cid' => $cid)),
            );
        }
//        if (!$cid && Yii::app()->user->id != ClaUser::getSupperAdmin()) {
//            return false;
//        }
//        if ($cid == ClaCategory::CATEGORY_ROOT && Yii::app()->user->id != ClaUser::getSupperAdmin()) {
//            return false;
//        }
        //
...
//            $parentmodel = PostCategories::model()->findByPk($pa);
//            if (!$parentmodel)
//                return false;
//            if ($parentmodel->site_id != $this->site_id)
//                return false;
//        }else if (Yii::app()->user->id != ClaUser::getSupperAdmin())
//            return false;
        //
        ...
        $post = Yii::app()->request->getPost('PostCategories');
        //
        ...
        if (Yii::app()->request->isPostRequest && $post) {
            ...
            if ($pa == 0) {
                ...
                $model2 = PostCategories::model()->findByPk($pa);
                if ($model2)
            ...
            if ($model->save()) {
                ...
                if ($isAjax) {
                    ...
                    if ($pa != ClaCategory::CATEGORY_ROOT)
                        $this->redirect(Yii::app()->createUrl('content/postcategory', array('cid' => $pa)));
                    else
                        $this->redirect(Yii::app()->createUrl("/content/postcategory"));
                }
            } else {
                if ($isAjax) {
        ...
        if ($pa != ClaCategory::CATEGORY_ROOT) {
            ...
            //breadcrumbs
            $this->breadcrumbs = array(
                $parentmodel->cat_name => Yii::app()->createUrl('/content/postcategory', array('cid' => $parentmodel->cat_id)),
                Yii::t('category', 'category_post_create') => Yii::app()->createUrl('content/postcategory/addcat', array('pa' => $pa)),
            );
        } else
        //breadcrumbs
            $this->breadcrumbs = array(
    ...
    public function actionEditcat($id) {
        ...
        $category->type = ClaCategory::CATEGORY_POST;
        //
        $cat = Yii::app()->request->getPost('PostCategories');
            ...
            if ($model->cat_id != $cat["cat_parent"]) {
                if ($model->cat_parent != $cat["cat_parent"]) {
                    if ($model->cat_parent != 0) {
                        $model_old_parent = PostCategories::model()->findByPk($model->cat_parent);   // Thư mục cha của thư mục hiện tại chưa được gán
                        if ($model_old_parent)
                            $model_old_parent->cat_countchild = $model_old_parent->cat_countchild - 1;
                    }

                    if ($cat["cat_parent"] != 0) {
                        $model_new_parent = PostCategories::model()->findByPk($cat["cat_parent"]);       // Thư mục cha được gán
                        if ($model_new_parent) {
                ...
                if ($model->save()) {
                    ...
                    if ($model->cat_parent != ClaCategory::CATEGORY_ROOT)
                        $this->redirect(Yii::app()->createUrl('content/postcategory', array('cid' => $model->cat_parent)));
                    else
                        $this->redirect(Yii::app()->createUrl("/content/postcategory"));
                }
            }
        ...
        $this->breadcrumbs = array(
            Yii::t('post', 'post_category') => Yii::app()->createUrl('/content/postcategory/'),
            Yii::t('category', 'category_post_update') => Yii::app()->createUrl('content/postcategory/editcat', array('id' => $id)),
        );
        //
        // If not post
        $category->generateCategory();
        ...
        if (Yii::app()->user->id != ClaUser::getSupperAdmin()) {
            ...
            //breadcrumbs
            $this->breadcrumbs = array(
                $cateparent['cat_name'] => Yii::app()->createUrl('/content/postcategory', array('cid' => $cateparent['cat_id'])),
                Yii::t('category', 'category_post_update') => Yii::app()->createUrl('content/postcategory/editcat', array('id' => $id)),
            );
            //
        }
    ...
    public function actionDelcat($id) {
        $model = PostCategories::model()->findByPk($id);
        if (!$model) {
