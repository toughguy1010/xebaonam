...
class RealestateCategoriesController extends BackController {
    ...
    public function actionCreate($id) {
        ...
        $this->breadcrumbs = array(
            Yii::t('realestate', 'realestate_category') => Yii::app()->createUrl('/content/realestateCategories/'),
            Yii::t('realestate', 'realestate_category_create') => Yii::app()->createUrl('/content/realestateCategories/create', array('id' => $id)),
        );
        ...
        $id = (int) $id;
        if ($id != 0) {
            $parent_model = RealEstateCategories::model()->findByPk($id);
        ...
        $model = new RealEstateCategories();
        $model->cat_parent = $id;
        
        $post = Yii::app()->request->getPost('RealEstateCategories');
        ...
        if (Yii::app()->request->isPostRequest && $post) {
            ...
            if ($id == 0) {
                ...
                $model2 = RealEstateCategories::model()->findByPk($id);
                if ($model2) {
            ...
            if ($model->save()) {
                ...
                $this->redirect(Yii::app()->createUrl("/content/realestateCategories"));
            }
    ...
    public function actionUpdate($id) {
        ...
        $this->breadcrumbs = array(
            Yii::t('realestate', 'realestate_category') => Yii::app()->createUrl('/content/realestateCategories/'),
            Yii::t('realestate', 'realestate_category_update') => Yii::app()->createUrl('/content/realestateCategories/update', array('id' => $id)),
        );
        ...
        $cat = Yii::app()->request->getPost('RealEstateCategories');
            ...
            if ($model->cat_id != $cat["cat_parent"]) {
                if ($model->cat_parent != $cat["cat_parent"]) {
                    if ($model->cat_parent != 0) {
                        $model_old_parent = RealEstateCategories::model()->findByPk($model->cat_parent);   // Thư mục cha của thư mục hiện tại chưa được gán
                        if ($model_old_parent) {
                    ...
                    if ($cat["cat_parent"] != 0) {
                        $model_new_parent = RealEstateCategories::model()->findByPk($cat["cat_parent"]);       // Thư mục cha được gán
                        if ($model_new_parent) {
    ...
    public function actionDelete($id) {
        ...
        $model = RealEstateCategories::model()->findByPk($id);
        if (!$model) {
    ...
    public function actionAdmin() {
        $model = new RealEstateCategories('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['RealEstateCategories']))
            $model->attributes = $_GET['RealEstateCategories'];
     ...
     * @return RealEstateCategories the loaded model
     ...
     * @param RealEstateCategories $model the model to be validated
     */
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'realestate-categories-form') {
    ...
    public function actionUploadfile() {
        if (isset($_FILES['file'])) {
            ...
            $up->setPath(array($this->site_id, 'realestatecategory', 'ava'));
            $up->uploadImage();
            $return = array();
