...
<div class="widget widget-box">
    <div class="widget-header"><h4>
            <?php echo Yii::app()->controller->action->id != "editcat" ? Yii::t('category', 'category_post_create') : Yii::t('category', 'category_post_update'); ?>
        </h4></div>
                    ...
                    'success': function(res) {
                        if (res.code != "200") {
                            ...
                            jQuery.ajax({
                                'type': 'POST',
                                'dataType': 'JSON',
                                'url': '<?php echo Yii::app()->createUrl('suggest/suggest/category', array('type' => ClaCategory::CATEGORY_SERVICE)); ?>',
                                'success': function(res) {
                                    if (res.code == '200') {
                                        if (res.html)
                                            jQuery('#SeServices_category_id').html(res.html);
...
<?php } ?>
        ...
        jQuery('#categoryavatar_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/content/postcategory/uploadfile"); ?>',
            name: 'file',
            onSubmit: function() {
            },
            onComplete: function(result) {
                ...
                if (obj.status == '200') {
                    if (obj.data.realurl) {
                        jQuery('#SeCategories_avatar').val(obj.data.avatar);
