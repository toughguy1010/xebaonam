...
<div class="row">
    <div class="col-xs-12 no-padding">
        ...
        <?php if ($user->bank_id) { ?>
            <div class="control-group form-group">
                <label class="col-sm-2 control-label no-padding-left">Hoặc tin thẻ ngân hàng</label>
                <div class="controls col-sm-10 right-content">
                    <?php
                    echo '<p>Số tài khoản ngân hà : ' . $user->bank_id . '</p>';
                    echo '<p>Tên ngân hàng : ' . $user->bank_name . '</p>';
                    echo '<p>Chi nhánh ngân hàng : ' . $user->bank_branch . '</p>';
                    ?>
                </div>
            </div>
        <?php } ?>
...
<script type="text/javascript">
    jQuery(function ($) {
        jQuery('#transfer_image_form').ajaxUpload({
            url: '<?php echo Yii::app()->createUrl("/expertransAffiliate/expertransAffiliateTransferMoney/uploadfile"); ?>',
            name: 'file',
            onSubmit: function () {
            },
