...
modalBox.getId = function () {
    return this.id;
};
modalBox.getHeader = function () {
    return this.header;
};
modalBox.getBody = function () {
    return this.body;
};
modalBox.getFooter = function () {
    return this.footer;
};
modalBox.getWidth = function () {
    return this.width;
};
modalBox.getHeight = function () {
    return this.height;
};
modalBox.getMoreClass = function () {
    return this.moreClass;
};
modalBox.setId = function (id) {
    this.id = id;
};
modalBox.setHeader = function (hd) {
    this.header = hd;
};
modalBox.setBody = function (bd) {
    this.body = bd;
};
modalBox.setFooter = function (ft) {
    this.header = ft;
};
modalBox.setWidth = function (width) {
    this.width = width;
};
modalBox.setHeight = function (height) {
    this.height = height;
};
modalBox.setMoreClass = function (cl) {
    this.moreClass = cl;
};
modalBox.show = function () {
    if (jQuery('#' + this.getId()).length) {
        if(this.getMoreClass()){
            jQuery('#' + this.getId()).addClass(this.getMoreClass());
        }
        jQuery('#' + this.getId()).find('.modal-dialog').css({width: this.getWidth() + 'px'});
        if (this.getHeight()) {
            jQuery('#' + this.getId()).find('.modal-dialog').css({height: this.getHeight() + 'px'});
        }
        jQuery('#' + this.getId()).find('.modal-header h4').html(this.getHeader());
        jQuery('#' + this.getId()).find('.modal-body').html(this.getBody());
        if (this.getFooter()) {
            jQuery('#' + this.getId()).find('.modal-footer').show();
            jQuery('#' + this.getId()).find('.modal-footer').html(this.getFooter());
        } else {
            jQuery('#' + this.getId()).find('.modal-footer').hide();
        }
        //
        jQuery('#' + this.getId()).removeClass('hide');
        jQuery('#' + this.getId()).modal('show');
    }
};
modalBox.hide = function () {
    jQuery('#' + this.getId()).addClass('hide');
    jQuery('#' + this.getId()).modal('hide');
};
modalBox.reset = function(){
    jQuery('#' + this.getId()).removeClass();
    jQuery('#' + this.getId()).addClass('modal fade in');
    this.setMoreClass('');
    this.setHeader('');
    this.setBody('');
    this.setFooter('');
