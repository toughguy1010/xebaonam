(function ($) {
    ...
    var Bonsai = function (el, options) {
        ...
        this.update();
        if (this.isRootNode()) {
            if (this.options.createCheckboxes)
                this.createInputs = 'checkbox';
            if (this.options.handleDuplicateCheckboxes)
                this.handleDuplicateCheckboxes();
            if (this.options.checkboxes)
                this.el.qubit(this.options);
            if (this.options.addExpandAll)
                this.addExpandAllLink();
            if (this.options.addSelectAll)
                this.addSelectAllLink();
    ...
    Bonsai.prototype = {
        ...
        update: function () {
            ...
            this.el.children().each(function () {
                var item = $(this);
                if (self.options.createInputs)
                    self.insertInput(item);
        ...
        serialize: function () {
            ...
            return this.el.find('li').toArray().reduce(function (acc, li) {
                ...
                if (id) {
                    ...
                    if (state)
                        acc[$li.hasClass('expanded') ? 'expanded' : 'collapsed'].push(id);
                }
                return acc;
        ...
        insertInput: function (listItem) {
            ...
            if (listItem.find('> input[type=' + type + ']').length)
                return;
        ...
        addExpandAllLink: function () {
            ...
            $('<div class="expand-all">')
                    .append(
                            $('<a class="all">Mở rộng tất cả</a>').css('cursor', 'pointer').on('click', function () {
                        self.expandAll();
                    })
                            )
                    .append('<i class="separator"></i>')
                    .append(
                            $('<a class="none">Thu gọn tất cả</a>').css('cursor', 'pointer').on('click', function () {
                        self.collapseAll();
                    })
                            )
        ...
        addSelectAllLink: function () {
            ...
            $('<div class="check-all">')
                    .append($('<a class="all">Chọn tất cả</a>')
                            .css('cursor', 'pointer')
                            .on('click', function () {
                                getCheckboxes().prop({
                                    checked: true,
                    ...
                    .append('<i class="separator"></i>')
                    .append($('<a class="none">Bỏ chọn tất cả</a>')
                            .css('cursor', 'pointer')
                            .on('click', function () {
                                getCheckboxes().prop({
                                    checked: false,
