...
class TokenGrid
{
    var $_x_grid_size=23;            // Horizontal grid size, alpha characters
    var $_y_grid_size=23;            // Vertical grid size, numeric chareacters
    var $_token_length=5;           // Token length
    ...
    public function TokenGrid()
    {
        $this->SetGridSize($this->_x_grid_size,$this->_y_grid_size);
        $this->SetTokenLength($this->_token_length);
        $this->SetGridSalt(date('H',time()));
        ...
        $this->SetNotCaseSensitive($this->_isSenitive);     // Initialize this value to the default one
    }
    ...
    {
        $this->_x_grid_size = $this->_x_grid_size;
        $this->_y_grid_size = $this->_y_grid_size;
    }
    ...
    function SetTokenLength()
    {
        $this->_token_length = (($this->_token_length < 1)?1:(($this->_token_length > 10)?10:$this->_token_length));
    }
    ...
    function GetRandomGridPosition($grid_id)
    {
		...
		for($i=0;$i<$this->randPost;$i++){
			array_push($arrPost,substr($this->_horizontal_grid_labels, mt_rand(1,$x_pos) - 1, 1).mt_rand(1,$y_pos));
		}
		return $arrPost;
    }
}
