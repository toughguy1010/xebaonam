<div class="colcenterask">
    <div class="menu-colcenterask">
        <ul class="clearfix">
            <li class="active"><a href="<?php echo Yii::app()->createUrl('economy/question/') ?>" title="Mới nhất">Mới nhất</a></li>
            <li><a href="<?php echo Yii::app()->createUrl('economy/question/', array('status' => ActiveRecord::STATUS_QUESTION_NOT_ANSWER)) ?>" title="Câu hỏi chưa được trả lời">Câu hỏi chưa được trả lời</a></li>
        </ul>
    </div>
    <div class="cont">
        <?php
        if (count($questions)) {
            foreach ($questions as $question) {
                ...
                <div class="item-colcenterask">
                    <div class="clearfix">
                        <?php if ($question['image_path'] && $question['image_name']) { ?>
                        ...
                        <div class="box-info">
                            <h4 class="name-ch"><a href="<?php echo $question['link'] ?>" title="<?php echo ($question['question_title'] != '') ? $question['question_title'] : $question['question_content'] ?>"><?php echo ($question['question_title'] != '') ? $question['question_title'] : $question['question_content'] ?></a></h4>
                             <p class="description-ch"><span class="admin-reply">Quản trị viên trả lời: </span><?php echo ($question['question_title'] != '') ? $question['question_content'] : '' ?></p>
                        </div>
                    </div>
