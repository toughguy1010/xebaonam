        ...
        <form action="" method="post" id='comment-form'>
            <div class="row">
                <div class="text col-sm-12" id="detail_wrapper">
                    ...
                    <textarea placeholder="Nhập nội dung tại đây. Tối thiểu 20 từ, tối đa 1000 từ." id="commentbox" onclick="saveContent()" name="detail" class="form-control"></textarea>
                    <div data-text="Enter text here" contentEditable="true" class="form-control div-input hide" name="detail" id="review_detail" cols="20" rows="3">
                    </div>
                    <span class="help-block" id="detail_error"></span>
                </div>
                <div class="comment-user hide">
                    <div style="padding: 15px;color: #0000FF;font-size: 11px;"><?php echo 'Để gửi bình luận bạn vui lòng nhập các thông tin bên dưới.' ?></div>
                    <div class="col-sm-6">
                        <!--<label for="review_name">* Tên:</label>-->
                        <input type="text" placeholder="Vui lòng nhập tên bạn" name="title" id="review_name" class="form-control input-sm" required="required" value="<?php echo ($user_profile->name) ? $user_profile->name : ''; ?>">
                        <span class="help-block" id="title_error"></span>
                    </div>
                    <div class="col-sm-6" id="detail_wrapper">
                        <!--<label for="review_email">* Email/Phone:</label>-->
                        <input type="text" placeholder="Email hoặc số điện thoại" name="title" id="review_email" class="form-control input-sm" required="required" value="<?php echo ($user_profile->email) ? $user_profile->email : ''; ?>"><span class="help-block" id="title_error"></span>
                    </div>
                </div>
                <div class="col-xs-6">
                                ...
                                <div class="user-cmt">
                                    <div class="user-cmt-avat">
                                        <div class="avatar-name-fword" style="">
                                            ...
                                            <?php } else { ?>
                                                <img src="<?php echo Yii::app()->theme->baseUrl ?>/css/img/logo_comment.png" width="100%" height="100%">
                                            <?php } ?>
                                        </div>
                                    </div>
                                </div>
    ...
    ?>
        <div class="pager">
            <ul id="yw1" class="W3NPager">
                <li class=" first_v hiden_page">
                    ....
                </li>
                <?php for ($i = 1; $i <= $total_page; $i++) { ?>
                    <li class="page <?php echo ($i === 1) ? 'selected' : '' ?> <?= ($i > 5) ? 'hiden_page' : '' ?> ">
                        <a href="javascript:void(0)" onclick="getCommentPage(<?php echo (int)$i; ?>, this)"><?php echo $i ?>
                        </a>
                    </li>
                <?php } ?>
                <?php if ($total_page > 5) { ?>
    ...
    function array_v() {
        $('.page').each(function(index) {
            if ($(this).hasClass("selected")) {
                $('.page').each(function(indexs) {
                    $(this).addClass("hiden_page");
                    if ((indexs + 3) > index && indexs < index) {
                        console.log(indexs);
                        $(this).removeClass("hiden_page");
                    }
                    if (indexs == index) {
                        console.log(indexs);
                        $(this).removeClass("hiden_page");
                    ...
                    if (indexs == 0) {
                        if ($(this).hasClass("hiden_page")) {
                            $(".first_v").removeClass("hiden_page");
                        }
                    }
                    if (index + 3 > $('.page').length) {
                        $(".first_v").addClass("hiden_page");
                    }
                });
            }
        });
