<div class="news_right">
	...
	<div id="newstab" class="show">
		<ul class="list_news">
			<?php foreach ($hotnews as $news) {
			    $src = ClaHost::getImageHost() . $news['image_path'] . 's100_100/' . $news['image_name'];
			    ?>
				<li>
					<a href="<?= $news['link']; ?>" title="<?= $news['news_title']; ?>">
                        <img src="<?=$src?>" alt="<?=$news['news_title']?>">
						<h3>
	...
	<?php
		$this->widget('common.widgets.wglobal.wglobal', array('position' => Widgets::POS_CENTER_BLOCK20));
	?>
	<?php
