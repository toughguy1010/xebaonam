                ...
                <td>
                    <?php if (!isset($update) || $update) { ?>
                        <form class="form-inline" role="form" action="<?php echo $this->createUrl('/economy/shoppingcart/update', array('key' => $key)); ?>" method="GET" enctype="multipart/form-data">
                            <input onchange="updateQuantity('<?php echo $key; ?>', $('#quantity-<?php echo $key; ?>').val());" id="quantity-<?php echo $key; ?>" type="number" class="form-control sc-quantity" max-lenght="3" value="<?php echo $shoppingCart->getQuantity($key); ?>" name="qty" min="1" step="1" max-lenght="3"/>
                            <a onclick="updateQuantity('<?php echo $key; ?>', $('#quantity-<?php echo $key; ?>').val());" class="btn btn-xs btn-primary"><i class="ico ico-refrest"></i></a>
                            <a href="<?php echo $this->createUrl('/economy/shoppingcart/delete', array('key' => $key)); ?>" class="btn btn-xs btn-danger"><i class="ico ico-delete"></i></a>
                        </form>
                    <?php } else { ?>
