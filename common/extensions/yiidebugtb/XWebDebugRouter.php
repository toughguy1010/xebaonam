...
define("warnning", 0.05);
define("warnning2", 0.009);
/*
 * Yii Debug Toolbar
 *
 * Using:
...
class yiiDebugClass {
    ...
    public static function render($items) {
        ...
        foreach ($items as $item) {
            ...
            $odd = !$odd;
            $timecheck = ($timest == 0) ? 0 : ($timestamp - $timest);
            $totaltime+=$timecheck;
            $style_color = "background-color: #F5F5F5;";
            if ($timecheck >= warnning)
                $style_color = "background-color: red;";
            elseif ($timecheck >= warnning2)
                $style_color = "background-color: #FFFF80;";

            if ($timest > 0) {
                $result.='<tr style="' . $style_color . '"><td colspan="4" align="center">' . "<img src='" . Yii::app()->baseUrl . "/images/arrowdown.png' height='30px' style='float: left;' /><span style='font-weight: bold;padding:6px 5px; display: inline-block'>Distance Time: " . $timecheck . "</span>" . '</td></tr>';
            }
        ...
        if ($result !== '') {
            $result.='<tr><td colspan="4">' . "<span style='color:blue; font-weight: bold;padding:6px 5px;'>Total Time: " . $totaltime . "</span>" . '</td></tr>';
            $result = '<tbody>' . $result . '</tbody>';
        }

        $result = '<table><thead><tr><th width="230px">Time</th><th>Level</th><th>Category</th><th width="60%">Message</th></tr></thead>' . $result . '</table>';
...
class YiiDebugUser extends yiiDebugClass {

    public $filter = null;

    public static function getInfo($data, $filter = null) {
        if ($filter === null)
            return array();
        parent::getInfo($data);
        $result = array();
        $result['title'] = 'User Log';
        $result['panelTitle'] = 'User Log:';
        $items = array();
        foreach ($data as $row) {
    ...
    public function getDebugInfo($data) {
        return self::getInfo($data, $this->filter);
    }
...
class XWebDebugRouter extends CLogRoute {
    ...
    public $ignoremodule = array('');
    public $userview = array(1, 237, 238,239, 249);
    ...
    public function processLogs($logs) {
        $app = Yii::app();
        $config = array();
//        if (!in_array((int) Yii::app()->user->id, $this->userview))
//            return;
//        if (MobileValidator::isMobile())
//            return;
        //if(in_array(Yii::app()->controller->module->id, $this->ignoremodule)) return;
        $pathinfo = $app->request->getPathInfo();
//                $pathinfo = explode("/", $pathinfo);
//                $pathinfo = $pathinfo[0];
        //if(in_array($pathinfo, $this->ignoremodule)) return;
        if ($this->ignoremodule) {
            foreach ($this->ignoremodule as $module) {
                if (strpos($pathinfo,$module)!==false) {
                    return;
                }
            }
        }
        ...
        //
        $YiiDebugUser = new YiiDebugUser();
        $YiiDebugUser->filter = "share";
        $items[] = $YiiDebugUser->getDebugInfo($logs);
