...
class searchbox extends WWidget {
    ...
    public function init() {
        ...
        // Load config
        $config_searchbox = new config_searchbox('', array('page_widget_id' => $this->page_widget_id));
        if ($config_searchbox) {
            $this->show_wiget_title = $config_searchbox->show_wiget_title;
            $this->widget_title = $config_searchbox->widget_title;
            $this->showcat = $config_searchbox->showcat;
            if(isset($config_searchbox->type)){
                $this->type = $config_searchbox->type;
            }
        }
        //
...
//        $this->keyWord = $parser->purify($this->keyWord); //we purify the $user_input
        ...
        //
        $this->action = Yii::app()->createUrl('/search/search/search');
        if($this->type){
            $this->action = Yii::app()->createUrl('/search/search/search',array(ClaSite::SEARCH_TYPE=>  $this->type));
        }
        // set name for widget, default is class name
...
//        }
    ...
    public function run() {
        $this->render($this->view, array(
            ...
            'show_widget_title' => $this->show_wiget_title,
