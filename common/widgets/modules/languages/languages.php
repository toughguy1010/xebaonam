<?php

/**
 * change language for site
 */
class languages extends WWidget {

    public $languages = array();
    protected $name = 'languages';
    protected $view = 'view';
    protected $autoTrans = 0;
    protected $actionKey = ''; // lưu tham số url hiện tại

    public function init() {
        // set name for widget, default is class name
        if ($this->name == '') {
            $this->name = get_class($this);
        }
        //
        // Load config
        $config_languages = new config_languages('', array('page_widget_id' => $this->page_widget_id));
        if (isset($config_languages->autoTrans)) {
            $this->autoTrans = $config_languages->autoTrans;
        }
        //
        if (!ClaSite::isMultiLanguage() && !$this->autoTrans) {
            return false;
        }
        if (isset($config_languages->widget_title))
            $this->widget_title = $config_languages->widget_title;
        if (isset($config_languages->show_wiget_title))
            $this->show_widget_title = $config_languages->show_wiget_title;
        if ($this->autoTrans) {
            $this->view.='_auto';
        }
        //
        $viewname = $this->getViewAlias(array(
            'view' => $this->view,
            'name' => $this->name,
        ));
        if ($viewname != '') {
            $this->view = $viewname;
        }
        //
        $flags = array(
            'vi' => 'data:image/png;base64,',
            'en' => 'data:image/png;base64,',
            'jp' => 'data:image/png;base64,',
        );
        //
        $module = isset(Yii::app()->controller->module->id) ? Yii::app()->controller->module->id . '/' : '';
        $controller = isset(Yii::app()->controller->id) ? Yii::app()->controller->id . '/' : '';
        $action = (isset(Yii::app()->controller->action->id) && Yii::app()->controller->action->id != 'index') ? Yii::app()->controller->action->id : '';
        $curentUrl = $module . $controller . $action;
        if ($curentUrl == Yii::app()->defaultController)
            $curentUrl = '/';
        //
        $this->actionKey = base64_encode(json_encode(array('url' => $curentUrl, 'params' => $_GET)));
        //
        $languages = ClaSite::getLanguagesForSite();
        unset($languages['vi']);
        $languages = ClaArray::AddArrayToBegin($languages, array('vi' => 'vi'));
        $currentLanguage = Yii::app()->language;
        //
        foreach ($languages as $language) {
            $this->languages[$language]['name'] = $language;
            $this->languages[$language]['displayName'] = Yii::t('languages', $language);
            $this->languages[$language]['key'] = $language;
            $this->languages[$language]['image'] = $flags[$language];
            $this->languages[$language]['link'] = Yii::app()->createUrl('/site/site/setlanguage', array(ClaSite::LANGUAGE_KEY => $language, ClaSite::LANGUAGE_ACTION_KEY => $this->actionKey));
            $this->languages[$language]['active'] = ($language == $currentLanguage) ? true : false;
        }
        //
        parent::init();
    }

    public function run() {
        $this->render($this->view, array(
            'languages' => $this->languages,
        ));
    }

}
