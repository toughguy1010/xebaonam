<?php
namespace common\components\alepay\Lib;
use common\components\alepay\Lib\Utils\AlepayUtils;

define('DS', str_replace('\\', '/', DIRECTORY_SEPARATOR));
define('ROOT_PATH_ALP', dirname(__FILE__));
include(ROOT_PATH_ALP . DS . 'Utils/AlepayUtils.php');
/*
 * Alepay class
...
class Alepay {
    ...
    protected $URI = array(
        ...
        'getInstallmentInfo' => '/checkout/v1/get-installment-info',
    );
    ...
    /*
     * get installment info from Alepay
     * @param array|null $data
     */
    public function getInstallmentInfo($data) {

        // demo data
        $url = $this->baseURL[$this->env] . $this->URI['getInstallmentInfo'];
        $result = $this->sendRequestToAlepay($data, $url);
        if ($result->errorCode == '000') {
            $dataDecrypted = $this->alepayUtils->decryptData($result->data, $this->publicKey);
            return  $dataDecrypted;
