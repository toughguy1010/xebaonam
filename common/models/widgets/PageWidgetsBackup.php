...
/**
 * This is the model class for table "page_widgets_backup".
 *
 * The followings are the available columns in table 'page_widgets_backup':
 ...
 */
class PageWidgetsBackup extends ActiveRecord {

    /**
     * @return string the associated database table name
     */
    public function tableName() {
        return $this->getTableName('page_widgets_backup');
    }
     ...
     * @return PageWidgetsBackup the static model class
     */
     ...
     */
    static function getAllPageWidgetBackup($options = array()) {
        //
        $site_id = isset($options['site_id']) ? $options['site_id'] : 0;
        //
        $data = Yii::app()->db->createCommand()->select()
            ->from(ClaTable::getTable('page_widgets_backup'))
            ->where("site_id=$site_id")
            ->queryAll();
        return $data;
    }

    static function getVersion($options = array()) {
        $site_id = isset($options['site_id']) ? $options['site_id'] : 0;
        $count = Yii::app()->db->createCommand()->select('COUNT(DISTINCT(version))')
            ->from(ClaTable::getTable('page_widgets_backup'))
            ->where("site_id=$site_id")
            ->queryScalar();
        return $count;
