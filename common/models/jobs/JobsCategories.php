...
class JobsCategories extends ActiveRecord {
    ...
    public function tableName() {
        return $this->getTableName('jobs_categories');
    }
    ...
    public function rules() {
        ...
        return array(
            ...
            array('user_id, cat_parent, cat_order, cat_countchild, status, created_time, modified_time, showinhome, limit', 'numerical', 'integerOnly' => true),
            array('cat_name, image_path, cover_path', 'length', 'max' => 255),
            array('alias', 'length', 'max' => 100),
            array('layout_action,view_action', 'length', 'max' => 100),
            array('alias', 'isAlias'),
            array('image_name, cover_name', 'length', 'max' => 200),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('cat_id, site_id, user_id, cat_parent, cat_name, alias, cat_order, cat_description, cat_countchild, image_path, image_name, status, created_time, modified_time, showinhome, meta_keywords, meta_description,meta_title,avatar, layout_action, view_action, cover_name, cover_path, cover, limit', 'safe'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            ...
            'cover_path' => 'Cat Image Path',
            'cover_name' => 'Cat Image Name',
    ...
    public static function getCategoryInHome($options = array()) {
        ...
        $categories = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('newcategory'))
                ...
                ->queryAll();
        $results = array();
        $category = new ClaCategory(array('type' => ClaCategory::CATEGORY_JOBS, 'create' => false));
    ...
    public static function getAllIdChildById($cat_id, $get_parent = false) {
        ...
        if($childs) {
            foreach ($childs as $key => $value) {
                if($value > 0) {
                    $tg = JobsCategoryChilds::getModel($value);
                    if($tg->jobs_category_list_child_all) {
                        $tg = explode(' ', $tg->jobs_category_list_child_all);
    ...
    public static function getIdChildsById($cat_id) {
        $childs = Yii::app()->db->createCommand()->select($select)->from(ClaTable::getTable('newcategory'))
    ...
    public function afterSave() {
        // $tg = JobsCategoryChilds::getModelbyCat($this, true);
        // $tg->save();
        parent::afterSave();
    }
