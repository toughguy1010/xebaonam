...
class NewsCategories extends ActiveRecord {
    ...
    public function tableName() {
        return $this->getTableName('newcategory');
    }
    ...
    public static function getCategoryInHome($options = array()) {
        ...
        $categories = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('newcategory'))
                ...
                ->queryAll();
        $results = array();
        $category = new ClaCategory(array('type' => ClaCategory::CATEGORY_NEWS, 'create' => false));
    ...
    public static function getAllIdChildById($cat_id, $get_parent = false) {
        ...
        if($childs) {
            foreach ($childs as $key => $value) {
                if($value > 0) {
                    $tg = NewsCategoryChilds::getModel($value);
                    if($tg->news_category_list_child_all) {
                        $tg = explode(' ', $tg->news_category_list_child_all);
    ...
    public function afterSave() {
        $tg = NewsCategoryChilds::getModelbyCat($this, true);
        $tg->save();
        parent::afterSave();
    }
