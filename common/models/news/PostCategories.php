...
class PostCategories extends ActiveRecord {
    ...
    public function tableName() {
        return $this->getTableName('postcategory');
    }
    ...
    public function rules() {
        ...
        return array(
            ...
            array('cat_name, cat_description, image_path, meta_keywords, meta_description', 'length', 'max' => 255),
    ...
    public function search() {
        ...
        $start = isset($this->cat_parent) ? $this->cat_parent : ClaCategory::CATEGORY_ROOT;
        $data = $category->createArrayDataProvider($start);
        //
    ...
    static function getAllCategory($site_id = null) {
        ...
        $categories = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('postcategory'))
    ...
    public static function getCategoryInHome($options = array()) {
        ...
        $categories = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('postcategory'))
                ...
                ->queryAll();
        $results = array();
        $category = new ClaCategory(array('type' => ClaCategory::CATEGORY_POST, 'create' => false));
