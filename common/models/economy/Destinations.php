...
class Destinations extends ActiveRecord
{
    ...
    {
        return $this->getTableName('destinations');
    }
    ...
    public function attributeLabels()
    {
        return array(
            'id' => 'ID',
            'name' => Yii::t('rent', 'destination_name'),
            'alias' => 'Alias',
            ...
            'ishot' => Yii::t('rent', 'destination_hot'),
    ...
    public static function getOptionsDestinations()
    {
        $data = Yii::app()->db->createCommand()->select('id, name')
            ->from(ClaTable::getTable('destinations'))
    ...
    {
        return Yii::app()->createUrl('/economy/rent/destination', array('id' => $this->id, 'alias' => $this->alias));
    }
     ...
     * @author : Hatv
     */
    public function getRentProduct($options = [])
    {
        $products = RentProduct::model()->findAllByAttributes(
            array(
        ...
        if (count($products)) {
            foreach ($products as $key => $product) {
                $data[$product->id] = $product->attributes;
                $data[$product->id]['link'] = Yii::app()->createUrl('/economy/rent/detail', array('id' => $product->id, 'alias' => $product->alias));
            }
        }
        return $data;
    }
     ...
     * @param type $options
     */
    public static function getAllDestinationInHome($options = array())
    {
        if (!isset($options['limit']))
            $options['limit'] = self::LIMIT;
        $siteid = Yii::app()->controller->site_id;
        $categories = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('destinations'))
        ...
        foreach ($categories as $cat) {
            $results[$cat['id']] = $cat;
            $results[$cat['id']]['link'] = Yii::app()->createUrl('economy/rent/destination/', array('id' => $cat['id'], 'alias' => $cat['alias']));
        }
        return $results;
    }
