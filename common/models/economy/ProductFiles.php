 ...
 */
class ProductFiles extends ActiveRecord {
    ...
    public function tableName() {
        return $this->getTableName('product_files');
    }
    ...
    public function rules() {
        ...
        return array(
            ...
            array('product_id, user_id, site_id, created_time, modified_time', 'numerical', 'integerOnly' => true),
            ...
            array('id, product_id, user_id, site_id, name, path, description, display_name, extension, size, created_time, modified_time, file_src, order', 'safe'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            'id' => 'ID',
            'product_id' => Yii::t('file', 'file_folder'),
            'user_id' => 'User',
            'site_id' => 'Site',
            'name' => 'Name',
            'path' => 'Path',
    ...
    static function searchFiles($options = array(), $countOnly = false) {
        ...
        if ($countOnly) {
            ...
            $count = $command->select($select)
                    ->from(ClaTable::getTable('product_files') . ' t')
...
// author: Viet
    static function getAllFile() {
        $siteid = Yii::app()->controller->site_id;
        $files = Yii::app()->db->createCommand()->select()->from(ClaTable::getTable('product_files'))
     ...
     * @return array
     */
    public static function getFiles($options = array(), $countOnly = false) {
        //
        $siteid = Yii::app()->controller->site_id;
        ...
        if (isset($options['product_id']) && $options['product_id']) {
            if (is_array($options['product_id'])) {
                $_fidString = '(' . implode(',', $options['product_id']) . ')';
                $condition.=' AND t.product_id IN' . $_fidString;
            } else {
        ...
        if (isset($options['limit']) && (int) $options['limit']) {
            ...
            $options[ClaSite::PAGE_VAR] = isset($options[ClaSite::PAGE_VAR]) ? $options[ClaSite::PAGE_VAR] : 1;
            $offset = ($options[ClaSite::PAGE_VAR] - 1) * $options['limit'];
        ...
        foreach ($data as $p) {
            $results[$p['id']] = $p;
            $results[$p['id']]['link'] = Yii::app()->createUrl('film/film/detail', array('id' => $p['id'], 'alias' => $p['alias']));
        }
        return $results;
    }
