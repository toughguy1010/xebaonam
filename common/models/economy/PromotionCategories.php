...
/**
 * This is the model class for table "promotion_categories".
 *
 * The followings are the available columns in table 'promotion_categories':
 ...
 */
class PromotionCategories extends ActiveRecord {
    ...
    public function tableName() {
        return $this->getTableName('promotion_categories');
    }
     ...
     * @return PromotionCategories the static model class
     */
    ...
    public static function getCategoryInHome($options = array()) {
        ...
        $categories = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('promotion_categories'))
                ...
                ->queryAll();
        $results = array();
        $category = new ClaCategory(array('type' => ClaCategory::CATEGORY_PROMOTION, 'create' => false));
        $category->application = 'public';
        foreach ($categories as $cat) {
            $results[$cat['cat_id']] = $cat;
            $results[$cat['cat_id']]['link'] = Yii::app()->createUrl('economy/promotion/category', array('id' => $cat['cat_id'], 'alias' => $cat['alias']));
            $results[$cat['cat_id']]['link2'] = Yii::app()->createUrl('economy/promotion/categoryunlimit', array('id' => $cat['cat_id'], 'alias' => $cat['alias']));
        }
        return $results;
    }
