...
class Manufacturer extends ActiveRecord {
    ...
    function getProduct($options = array(), $countOnly = false){
        ...
        // Full text lọc theo cat
        if (isset($options[ClaCategory::CATEGORY_KEY]) && $options[ClaCategory::CATEGORY_KEY]) {
            $where .= " AND MATCH (category_track) AGAINST ('+". $options[ClaCategory::CATEGORY_KEY] .' -29470' .  "' IN BOOLEAN MODE)";
        }
        else{
            $where .= " AND NOT MATCH (category_track) AGAINST ('29470' IN BOOLEAN MODE)";
        }
     ...
     * @return type
     */
    static function getManufacturerArr()
    {
        $results = array();
    ...
    public static function getCategoryByTrack($track,$category) {
        ...
        if (isset($track) && $track) {
            ...
            $data = Yii::app()->db->createCommand()->select('*')
                ->from(ClaTable::getTable('manufacturers'))
                ->where('site_id=:site_id AND id IN (' . join(',', $trackCondition) . ')', [':site_id' => $siteid])
                ->queryAll();
        }
        $result = [];
        if (isset($data) && $data) {
            foreach ($data as $item) {
                $result[$item['id']] = $item;
                $result[$item['id']]['link'] = Yii::app()->createUrl('/economy/product/category', array('id' => $category->cat_id, 'alias' => $category->alias,'mnftr_id' => $item['id']));;
            }
        }
        return $result;
    }
