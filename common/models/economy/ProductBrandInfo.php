 ...
 */
class ProductBrandInfo extends ActiveRecord {
    ...
    public function tableName() {
        return $this->getTableName('product_brand_info');
    }
    ...
    public function rules() {
        ...
        return array(
            array('product_brand_id, site_id', 'length', 'max' => 11),
            array('shortdes, address', 'length', 'max' => 250),
            array('phone', 'length', 'max' => 20),
            array('meta_title, meta_keywords, meta_description', 'length', 'max' => 255),
            array('product_brand_id, site_id, shortdes, description, address, phone, meta_title, meta_keywords, meta_description', 'safe'),
        );
    }
     ...
     * @return ProductBrandInfo the static model class
     */
     ...
     * @return array
     */
    public static function getProductBrandInfoInSite($options = array()) {
        ...
        $manuInfos = Yii::app()->db->createCommand()->select('*')->from(ClaTable::getTable('product_brand_info'))
                ->where("site_id=$siteid")
                ->limit($options['limit'], $offset)
                ->queryAll();
        $results = array();
        foreach ($manuInfos as $m) {
            $results[$m['product_brand_id']] = $m;
        }
        return $results;
    }
