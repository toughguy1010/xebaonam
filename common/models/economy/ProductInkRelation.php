 ...
 */
class ProductInkRelation extends CActiveRecord
{
//    public $id;
     ...
     */
    static function getProductIdInInkRel($product_id) {
     ...
     * @param type $product_id
     */
    static function countProductInInkRel($product_id)
    {
        $product_id = (int)$product_id;
     ...
     * @param array $options
     */
    static function getProductInInkRel($product_id, $options = array())
    {
        $product_id = (int)$product_id;
        ...
        if ($product_id) {
            $products = Yii::app()->db->createCommand()->select()
                ->from(ClaTable::getTable('product_ink') . ' pg')
    ...
    public function SearchProducts()
    {
        ...
        if (!$page)
            $page = 1;
        $products = ProductInkRelation::model()->findAllByAttributes(array('site_id' => Yii::app()->siteinfo['site_id'], 'product_id' => $this->product_id), array('limit' => $pagesize * $page));
        return new CArrayDataProvider($products, array(
            'pagination' => array(
                'pageSize' => $pagesize,
                'pageVar' => ClaSite::PAGE_VAR,
            ),
            'totalItemCount' => ProductInkRelation::countProductInInkRel($this->product_id),
        ));
    }
