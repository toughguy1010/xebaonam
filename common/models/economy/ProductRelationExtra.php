...
/**
 * This is the model class for table "product_relation_extra".
 *
 * The followings are the available columns in table 'product_relation_extra':
 ...
 */
class ProductRelationExtra extends CActiveRecord {
//    public $id;
     ...
     * @return ProductRelationExtra the static model class
     */
    ...
    static function getProductIdInRel($product_id) {
        ...
        $products = Yii::app()->db->createCommand()->select()
                ->from(ClaTable::getTable('product_relation_extra'))
    ...
    static function getProductInRel($product_id, $options = array()) {
        ...
        if ($product_id) {
            $products = Yii::app()->db->createCommand()->select()
                    ->from(ClaTable::getTable('product_relation_extra') . ' pg')
    ...
    public function SearchProducts() {
        ...
        if (!$page)
            $page = 1;
        $products = ProductRelationExtra::model()->findAllByAttributes(array('site_id' => Yii::app()->siteinfo['site_id'], 'product_id' => $this->product_id), array('limit' => $pagesize * $page));
        return new CArrayDataProvider($products, array(
            'pagination' => array(
                'pageSize' => $pagesize,
                'pageVar' => ClaSite::PAGE_VAR,
            ),
            'totalItemCount' => ProductRelationExtra::countProductInRel($this->product_id),
        ));
    }
