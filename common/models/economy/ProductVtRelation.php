 ...
 */
class ProductVtRelation extends CActiveRecord
{
//    public $id;
    ...
    public function rules()
    {
        ...
        return array(
            array('site_id, created_time', 'numerical', 'integerOnly' => true),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('product_rel_id, site_id, created_time', 'safe'),
        );
    }
     ...
     */
    static function getProductIdInVtRel($product_id,$rel_products=false) {
        $product_id = (int) $product_id;
        $condition = 'site_id=' . Yii::app()->siteinfo['site_id'] . " AND MATCH (product_rel_id) AGAINST ('" . $product_id . "' IN BOOLEAN MODE)";
        if ($rel_products) {
            $rel_products = explode(',',$rel_products);
            foreach ($rel_products as $rel_id) {
                $condition .= " OR MATCH (product_rel_id) AGAINST ('" . $rel_id . "' IN BOOLEAN MODE)";
     ...
     * @param type $product_id
     */
    static function countProductInVtRel($product_id)
    {
        $product_id = (int)$product_id;
     ...
     * @param array $options
     */
    static function getProductInVtRel($product_id, $options = array())
    {
        $product_id = (int)$product_id;
        ...
        if ($product_id) {
            $products = Yii::app()->db->createCommand()->select()
                ->from(ClaTable::getTable('product_vt') . ' pg')
                ->join(ClaTable::getTable('product') . ' p', 'pg.product_rel_id=p.id')
                ->where('pg.site_id=' . Yii::app()->siteinfo['site_id'] . ' AND product_id=' . $product_id)
                ->limit($options['limit'])
    ...
    public function SearchProducts()
    {
        ...
        if (!$page)
            $page = 1;
        $products = ProductVtRelation::model()->findAllByAttributes(array('site_id' => Yii::app()->siteinfo['site_id'], 'product_id' => $this->product_id), array('limit' => $pagesize * $page));
        return new CArrayDataProvider($products, array(
            'pagination' => array(
                'pageSize' => $pagesize,
                'pageVar' => ClaSite::PAGE_VAR,
            ),
            'totalItemCount' => ProductVtRelation::countProductInVtRel($this->product_id),
        ));
    }

    public function getIdInRel($id = false)
    {
        $results = [];
        ...
        $rel = Yii::app()->db->createCommand()->select()
            ->from(ClaTable::getTable('product_vt'))
            ->where('site_id=' . $site_id . " AND MATCH (product_rel_id) AGAINST ('" . $id . "' IN BOOLEAN MODE)")
            ->queryRow();
        if (!count($rel) || !$rel) {
            return $results;
        }
        $rel_track = str_replace(' ', ',', $rel['product_rel_id']);
        $products = Yii::app()->db->createCommand()->select('id,alias,barcode,name,code,price')
            ->from(ClaTable::getTable('product'))
            ->where('site_id=' . $site_id . " AND id in (" . $rel_track . ")")
            ->queryAll();
        foreach ($products as $product) {
            $results[$product['id']] = $product;
            $results[$product['id']]['link'] = Yii::app()->createUrl('economy/product/detail',['id'=>$product['id'],'alias' => $product['alias']]);
        }
        return $results;
