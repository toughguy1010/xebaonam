...
class Files extends ActiveRecord {
    ...
    public function tableName() {
        return $this->getTableName('files');
    }
    ...
    public function rules() {
        ...
        return array(
            ...
            array('folder_id, user_id, site_id, created_time, modified_time', 'numerical', 'integerOnly' => true),
            ...
            array('description, publicdate_time', 'safe'),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, folder_id, user_id, site_id, name, path, description, display_name, extension, size, created_time, modified_time, file_src', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function attributeLabels() {
        return array(
            'id' => 'ID',
            'folder_id' => Yii::t('file', 'file_folder'),
            'user_id' => 'User',
            'site_id' => 'Site',
            'name' => 'Name',
            'path' => 'Path',
    ...
    public function search($options = array()) {
        ...
        if(isset($_GET['publicdate_time']) && $_GET['publicdate_time'] > 1000 &&  $_GET['publicdate_time'] < 9999) {
            $time_min = strtotime('1-1-'.$_GET['publicdate_time']);
            $time_max = strtotime('1-1-'.($_GET['publicdate_time']+1));
            $criteria->addCondition("(publicdate_time >= '$time_min' AND publicdate_time < '$time_max')");
        } else {
            $criteria->compare('publicdate_time', $this->publicdate_time);
        }
     ...
     * @param type $options
     */
    static function searchFiles($options = array(), $countOnly = false) {
        $results = array();
...
// author: Viet
    ...
    // Get time Publicdate min
    // author: Cong
    static function getMinPublicdateTime() {
        $siteid = Yii::app()->controller->site_id;
        $file = Yii::app()->db->createCommand()->select()->from(ClaTable::getTable('files'))
                ->where("site_id=$siteid AND publicdate_time > 0")
                ->order('publicdate_time ASC')
                ->limit(1)
                ->queryAll();
        return $file ? $file[0]['publicdate_time'] : time();
    }

    static function getFileInFolder($folder_id = 0, $options = array()) {
        ...
        if ($folder_id) {
            ...
            $condition = 'site_id=:site_id AND folder_id=:folder_id';
            $params = array(
                ':site_id' => Yii::app()->siteinfo['site_id'],
                ':folder_id' => $folder_id,
            );
            $files = Yii::app()->db->createCommand()->select()
                ->from(ClaTable::getTable('files'))
                ->where($condition, $params)
                ->limit($options['limit'])
