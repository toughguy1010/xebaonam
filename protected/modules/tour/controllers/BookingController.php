...
class BookingController extends PublicController {
    ...
    public function actionBookingTourNew($tour_id)
    {
        ...
        $listprovince = TourTouristDestinations::getOptionsDestinations();
        ...
        if ($tour) {
            ...
            if (isset($_POST['TourBooking']) && $_POST['TourBooking']) {
                ...
                if ($model->save()) {
                    ...
                    if ($mailSetting) {
                        // Chi tiết trong thư
                        $order_prd = $this->renderPartial('_booking_mail_settings', array(
                            'model' => $model,
                            'tour' => $tour,
                        ), true);
                        //
    ...
    public function actionBookingTourAll()
    {
        if (!isset($_POST['TourBooking']['qty'])) {
            $qty = 1;
        }
        $category = TourCategories::model()->findByPk($tour->tour_category_id);
        $listprovince = TourTouristDestinations::getOptionsDestinations();
        $liststar = TourHotelGroup::getOptionsGroup();
        $model = new TourBooking();
...
//                }
                if ($mailSetting) {
                    // Chi tiết trong thư, gửi tới khách hàng
                    $order_prd = $this->renderPartial('_booking_mail_settings', array(
                        'model' => $model,
                        ), true);
                    //
        ...
        $this->render('booking_tour_all', array(
            'model' => $model,
            'tour' => $tour,
            'qty' => $qty,
            'category' => $category,
