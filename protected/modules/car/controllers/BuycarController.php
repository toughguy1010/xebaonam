...
class BuycarController extends PublicController
{
    ...
    public function actionRegionPrice () {
        $id = Yii::app()->request->getParam('id', 0);
        if ($id) {
            $region = CarReceiptFee::model()->findByPk($id);
            $region_price = [];
            $region_price['name'] = $region->name;
            $region_price['number_plate_fee'] = $region->number_plate_fee;
            $region_price['registration_fee'] = $region->registration_fee;
            $region_price['inspection_fee'] = $region->inspection_fee;
            $region_price['road_toll'] = $region->road_toll;
            $region_price['insurance_fee'] = $region->insurance_fee;
    ...
    public function actionCalculateCostToyotaStep3()
    {
        ...
        if (isset($region) && $region) {
            $region_price = CarReceiptFee::model()->findByPk($region);
            $car->number_plate_fee = $region_price->number_plate_fee;
            $car->road_toll = $region_price->road_toll;
            $car->insurance_fee = $region_price->insurance_fee;
            $car->registration_fee = $region_price->registration_fee;
            $car->inspection_fee = $region_price->inspection_fee;
        }
    ...
    public function actionSendMailCalculateCostToyota()
    {
        ...
        $email_admin = Yii::app()->siteinfo['admin_email'];
        $name = isset($_POST['name']) ? $_POST['name'] : '';
        $key_send_mail = isset($_POST['key_send_mail']) ? $_POST['key_send_mail'] : '';
        if ($email_admin && $key_send_mail) {
            $data = $_SESSION[$key_send_mail];
            $data['name_kh'] = $name;
            $data['email_kh'] = $email;
            // Send mail
            $subject = 'Dự toán chi phí';
            $content = $this->renderPartial('mail/mail-admin-costToyota', $data, array('name' => $name, 'email' => $email), true);
            Yii::app()->mailer->send('', $email_admin, $subject, $content);
        }
    ...
    public function actionCaltoyata_v2()
    {
        $car_id = Yii::app()->request->getParam('car_id', 0);
        $sale = Yii::app()->request->getParam('sale', 0);
        $interest_id = Yii::app()->request->getParam('interest_id');
        $car_suport_type = Yii::app()->request->getParam('car_suport_type', 1);
        $month = Yii::app()->request->getParam('car_suport_year', 0) * 12;
        $car_earnest = $this->toNmuber(Yii::app()->request->getParam('car_earnest', 0));
        $car_prepay = Yii::app()->request->getParam('car_prepay_val', 0.1);
        $car_price = $this->toNmuber(Yii::app()->request->getParam('car_price', 0));
        if ($sale/$car_price>0.5) {
            echo "Số tiền nhập vào không hợp lệ";
            return false;
        }
        $car_component_price = $this->toNmuber(Yii::app()->request->getParam('car_component_price', 0));
        $total_price = $car_price + $car_component_price - $sale;
        $interest = CarInterest::getAllRegional()[0]['before_one']; //Lấy giá trị trả góp đầu tiên
        $interest_after = CarInterest::getAllRegional()[0]['after_one']; //Giá trị trả góp sau 1 năm
        if (isset($interest_id) && $interest_id) {
            $interest1 = CarInterest::model()->findByPk($interest_id); //Lấy theo giá trị đã chọn
            $interest = $interest1->before_one;
            $interest_after = $interest1->after_one;
        }
//        if ($month > 12) {
//            $interest = CarSuport::optionInterests()[2];
//        }
        switch ($car_suport_type) {
            case 1:
                if ($car_id && $car_prepay && $total_price && $month) {
                    $price = $total_price * (1-$car_prepay);
                    $array_month = range(1, $month, 1);
                    $array_money = array();
                    $principal = round($price / $month);
                    foreach ($array_month as $v) {
                        if ($v > 12) {
                            $interest = $interest_after; //Giá trị lãi suất sau 1 năm
                        }
                        $array_money[$v]['principal'] = $principal; // Tiền gốc phải nộp mỗi tháng
                        $array_money[$v]['recover_money'] = round($price - $principal); // Dư nợ
                        $array_money[$v]['interest'] = round((($price / 100) * $interest) / $month); // Tiền lãi
    ...
    public function actionCarReceiptFee() {
        $receipt_id = Yii::app()->request->getParam('id');
        if (isset($receipt_id) && $receipt_id) {
            $receipt = CarReceiptFee::model()->findByPk($receipt_id);
            $data = [];
            $data['number_plate_fee'] = $receipt->number_plate_fee;
            $data['registration_fee'] = $receipt->registration_fee;
            $data['inspection_fee'] = $receipt->inspection_fee;
            $data['road_toll'] = $receipt->road_toll;
            $data['insurance_fee'] = $receipt->insurance_fee;
            $this->jsonResponse(200, array(
                'data' => $data,
            ));
        }
    ...
    public function actionSupportBuyToyota()
    {
        ...
        $components['prepay'] = CarSuport::optionPrepay();
        $components['type'] = CarSuport::optionTypes();
        $components['interest'] = CarSuport::optionInterests();
        $components['interest_v2'] = CarInterest::getAllRegional();
