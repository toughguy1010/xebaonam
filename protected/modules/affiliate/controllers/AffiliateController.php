...
class AffiliateController extends PublicController {
    ...
    public function actionReportOrder() {
        ...
        // Đơn hàng hoàn thành
        $orderCompleteCount = AffiliateOrder::countOrder(Orders::ORDER_COMPLETE, $user_id, $options);
        // Hoa hồng đơn hàng hoàn thành
        // Đơn hàng hủy
        $orderDestroyCount = AffiliateOrder::countOrder(Orders::ORDER_DESTROY, $user_id, $options);
        //
        $rate = ($orderWaitingCount + $orderCompleteCount) / ($clickCount / 100);
        // Get commission order success
        $orderItems = AffiliateOrderItems::getAllOrderItem($user_id, $options);
        $commission = AffiliateOrderItems::calculatorCommission($orderItems);
        //
        // Get orders items
