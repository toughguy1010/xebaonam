...
class AttributeHelper extends BaseHelper {
    ...
    const MAX_ATTRIBUTE_VALUE_FIELD = 3;
        ...
        if (!empty($dynamic)) {
            foreach ($dynamic as $objAtt) {
                if (isset($attributesShow[$objAtt->id])) {
                    $results[$objAtt->id]['avatar_path'] = $attributesShow[$objAtt->id]['avatar_path'];
                    $results[$objAtt->id]['avatar_name'] = $attributesShow[$objAtt->id]['avatar_name'];
                    ...
                    if (isset($objAtt->value_child) && $objAtt->value_child) {
                        ...
                        $results[$objAtt->id]['index_key'] =  $objAtt->index_key;
    ...
    function getConfigurableAttributes($att_set_id = 0) {
        $atttibutes = array();
        $con = 'site_id=' . Yii::app()->controller->site_id . ' AND (is_configurable=1 AND attribute_set_id=:attribute_set_id) OR is_system=1 order by sort_order asc limit 3';
        $params = array(':attribute_set_id' => $att_set_id);
    ...
    function geProducttConfigurableValues($product = array(), $isArray = true) {
        ...
        if (!$isArray) {
            $proConfigs = ProductConfigurableValue::model()->findAll(
                    'site_id=' . Yii::app()->controller->site_id . ' AND product_id=:product_id ORDER BY `order` ASC, id ASC limit 999', array(':product_id' => $product['id'])
            );
        } else {
            $proConfigs = Yii::app()->db->createCommand()->select()->from(ProductConfigurableValue::model()->tableName())
                    ->where('site_id=' . Yii::app()->controller->site_id . ' AND product_id=:product_id ORDER BY `order` ASC, id ASC', array(':product_id' => $product['id']))
                    ->limit(999)
                    ->queryAll();
        }
        if (count($proConfigs)) {
     ...
     */
    function getAllProductConfigurable($product, $options = []) {
        ...
        foreach ($product_configurable as $k => $prd) {
            $products[$k] = $prd;
            $products[$k]['price_format'] = number_format($prd['price'], 0, ',', '.');
            $products[$k]['label_ext'] = '';
            $products[$k]['attribute_code'] = '';
            if (isset($prdc->attribute1_id) && $prdc->attribute1_id > 0) {
                $products[$k]['attibute1'] = $data_options[$prdc->attribute1_id][$prd['attribute1_value']];
                // minhbn: Merge attribes to array
                if (isset($options['attributesArray'])) {
                    $products[$k]['attibutes']['attibute1'] = $products[$k]['attibute1'];
                }
            ...
            if (isset($prdc->attribute2_id) && $prdc->attribute2_id > 0) {
                $products[$k]['attibute2'] = $data_options[$prdc->attribute2_id][$prd['attribute2_value']];
                 // minhbn: Merge attribes to array
                if (isset($options['attributesArray'])) {
                    $products[$k]['attibutes']['attibute2'] = $products[$k]['attibute2'];
                }
            ...
            if (isset($prdc->attribute3_id) && $prdc->attribute3_id > 0) {
                $products[$k]['attibute3'] = $data_options[$prdc->attribute3_id][$prd['attribute3_value']];
                 // minhbn: Merge attribes to array
                if (isset($options['attributesArray'])) {
                    $products[$k]['attibutes']['attibute3'] = $products[$k]['attibute3'];
                }
     ...
     * @param type $product
     */
    function getConfiguableValuesActive($att_set_id = 0, $product = array()) {
        $results = array();
        // các attributes dc set là configurable của
        $attributeConfigurables = AttributeHelper::helper()->getConfigurableAttributes($att_set_id);
        //Các giá trị configurable của product
        ...
        $activeConfigurableField = 0;
        foreach ($attributeConfigurables as $attribute_id => $attribute) {
            if ($attribute['field_configurable']) {
                $configurableFields[$attribute['field_configurable']] = $attribute;
                if ($attribute['type_option'] == ProductAttribute::TYPE_OPTION_COLOR) {
                    $activeConfigurableField = 'attribute' . $attribute['field_configurable'] . "_value";
                }
            }
        }
        foreach ($productConfigurableValues as $config) {
            $activeValue = isset($config[$activeConfigurableField]) ? $config[$activeConfigurableField] : 0;
            $configuables[$activeValue][$config['id']] = $config;
            for ($i = 1; $i <= self::MAX_ATTRIBUTE_VALUE_FIELD; $i++) {
                $configurableField = 'attribute' . $i . "_value";
                $configurableFieldName = 'attribute' . $i . "_value_name";
                if ($config[$configurableField]) {
                    $configuables[$activeValue][$config['id']][$configurableFieldName] = $this->getOptionNameByIndexKey($config[$configurableField], $configurableFields[$i]['options']);
                    $configuables[$activeValue][$config['id']][$configurableField . '_ext'] = $this->getOptionExtByIndexKey($config[$configurableField], $configurableFields[$i]['options']);
                    $configuables[$activeValue][$config['id']][$configurableField . '_text'] = $this->getOptionTextByIndexKey($config[$configurableField], $configurableFields[$i]['options']);
                }
            }
        }
        //
        if ($configuables) {
            $results['attributes'] = $attributeConfigurables;
            $results['configuables'] = $configuables;
        }
        return $results;
    }

    //
    function getOptionExtByIndexKey($index_key_value = '', $attributeOptions = array()) {
        foreach ($attributeOptions as $option)
            if ($option['index_key'] == $index_key_value)
                return isset($option['ext']) ? $option['ext'] : $option['ext'];
        return '';
    }
