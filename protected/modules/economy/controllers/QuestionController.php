...
class QuestionController extends PublicController
{
    ...
    public function actionIndex($status = null)
    {
        ...
        //
        $page = (Yii::app()->request->getParam(ClaSite::PAGE_VAR)) ? Yii::app()->request->getParam(ClaSite::PAGE_VAR) : 0;
        $pageSize = QuestionHelper::helper()->getPageSize();
        $options = array('limit' => $pageSize, ClaSite::PAGE_VAR => $page, 'status' => $status);
        ...
        //
        $questions = QuestionAnswer::getQuestions($options);
        $totalitem = QuestionAnswer::getQuestions($options, true);
        //
     ...
     * Index
     */
    public function actionProductquestion($id = null)
    {
        //
        if (!$id) {
        ...
        //
        $modalProduct = Product::model()->findByPk($id);
        if (!$modalProduct) {
            $this->sendResponse(404);
        }
        if ($modalProduct->site_id != $this->site_id) {
            $this->sendResponse(404);
        }
        //
        $this->breadcrumbs = array(
        ...
        // Metatag doing....
        $this->pageTitle = Yii::t('question','question') . $modalProduct->name;
        //
        $questions = QuestionAnswer::getQuestions($options);
        $totalitem = QuestionAnswer::getQuestions($options, true);
        //
        $this->render('index', array(
            'product' => $modalProduct->attributes,
            'questions' => $questions,
            'totalitem' => $totalitem,
            'limit' => $pageSize,
        ));
...
//            $categoryClass = new ClaCategory(array('type' => ClaCategory::CATEGORY_QUESTION, 'create' => true));
