...
//        $('#algalley .alimglist').masonry('layout');
    ...
    jQuery('#Product_product_category_id').on('change', function() {
    ...
    jQuery('#pro-att-t').on('click', function() {
        ...
        if (jQuery.data(document.body, "ajax_load_attribute") === undefined && !product_id) {
            jQuery.data(document.body, "ajax_load_attribute", 1);
        }
        if (jQuery.data(document.body, "ajax_load_attribute")) {
            jQuery.ajax({
                url: '<?php echo Yii::app()->createUrl('economy/productManager/ajaxLoadAttribute'); ?>',
                type: 'POST',
                dataType: 'html',
                data: {'product_id': product_id, 'category_id': category_id},
                beforeSend: function() {
                    jQuery('.product-attribute-tab').html("Loading ...");
                    w3ShowLoading(thi, 'right', 40, 0);
                },
                success: function(data) {
                    jQuery('.product-attribute-tab').html(data);
