<style>
    ...
    #product-attributes-cf .action{line-height: 30px;padding-top: 5px;display: block;width: 268px;}
    /*    #product-attributes-cf .action *{font-size: 12px;}*/
    #product-attributes-cf .action i{margin: 10px 5px; cursor: pointer; float: left;width: 100px;display: inline-block}
...
<script type="text/javascript">
    ...
    var tmp_att_1 = '<?php if (isset($attributes_cf[0])) echo str_replace(array('\r\n', "\n"), '', CHtml::dropDownList('attribute_cf[new][count_new][' . $attributes_cf[0]->field_configurable . ']', '', CHtml::listData($attributes_cf[0]->getAttributeOption(), 'index_key', 'value'), array('empty' => '-- Lựa chọn --', 'class' => '', 'style' => 'width:90%;'))); ?>';
    var tmp_att_2 = '<?php if (isset($attributes_cf[1])) echo str_replace(array('\r\n', "\n"), '', CHtml::dropDownList('attribute_cf[new][count_new][' . $attributes_cf[1]->field_configurable . ']', '', CHtml::listData($attributes_cf[1]->getAttributeOption(), 'index_key', 'value'), array('empty' => '-- Lựa chọn --', 'class' => '', 'style' => 'width:90%;'))); ?>';
    var type_option = "";
    var htmlOp = '';
...
//        htmlOp += '<div class="col-sm-2 att-pro-price"><input type="text" id="attribute_cf_new_4" class="numberFormat" name="attribute_cf[new][' + count_new + '][4]" value="" style="width:90%;" maxlength="15"></div>';
        ...
        htmlOp += '<span class=\"col-sm-1 help-inline action\"><i class=\"addattri icon-plus\">Thêm màu</i><i class=\"removeattri icon-minus\">Bỏ màu</i></span>';
        ...
        $.getJSON(
                'economy/productManager/renderImageConfig',
                {'count_new': count_new},
                function (data) {
