<?php 
    ...
    if(isset($attributesShow[$att_carat]['value']) && $attributesShow[$att_carat]['value']>0 && $pro['price']){
        $price_per_carat = round($pro['price']/floatval($attributesShow[$att_carat]['value']));
    }
    $rate_sale = 0;
    if($price_per_carat && isset($attributesShow[$att_price_stand]['value']) && $attributesShow[$att_price_stand]['value']>0){        
        $rate_sale = round((floatval($attributesShow[$att_price_stand]['value']) - $price_per_carat)/floatval($attributesShow[$att_price_stand]['value']),2)*100;
    }
?>
<?php if(count($attributesShow)){?>
    <div class="hover-table-sp-sp">
        ...
        <div class="ct-tt-hover-table-sp-sp">
            <ul>                  
                <li>Mã kim cương : <span style="padding-left:5px;"><?php echo $pro['code'];?></span></li>
                ...
                <li>
                    ...
                    <span style="padding-left:4px;">
                        ...
                        if(is_array($attribute['value'])){
                            ...
                            echo ($attribute['frontend_input']=='price')?HtmlFormat::money_format(floatval($attribute['value']))." đ":$attribute['value'];
                        }?>
                    </span>
                </li>                    
                <?php }?>
                <?php if($price_per_carat){?>
                    <li>Giá (carat) : <span style="padding-left:8px;"><?php echo HtmlFormat::money_format(floatval($price_per_carat)). " đ";?></span></li>
                <?php }?>
                <?php if($rate_sale>0){ ?>
