            ...
            if (!$model->getErrors()) {
                if ($model->save()) {
                    ...
                    if (isset($_POST['ShopStore'])) {
                        $stores = $_POST['ShopStore'];
                        if (count($stores)) {
                            foreach ($stores as $store) {
                                $model_store = new ShopStore();
                                $model_store->shop_id = $model->id;
                                $model_store->address = $store['address'];
                                $model_store->province_id = $store['province_id'];
                                $province = LibProvinces::getProvinceDetail($model_store->province_id);
                                if (isset($province) && $province['name']) {
                                    $model_store->province_name = $province['name'];
                                }
                                $model_store->district_id = $store['district_id'];
                                $district = LibDistricts::getDistrictDetailFollowProvince($model_store->province_id, $model_store->district_id);
                                if (isset($district) && $district['name']) {
                                    $model_store->district_name = $district['name'];
                                }
                                $model_store->ward_id = $store['ward_id'];
                                $ward = LibWards::model()->findByPk($model_store->ward_id);
                                if ($ward) {
                                    $model_store->ward_name = $ward->name;
                                }
                                $model_store->phone = $store['phone'];
                                $model_store->hotline = $store['hotline'];
                                $model_store->save();
                            }
                        }
                    }
                    // END SAVE ADDRESS STORE
