...
<script type="text/javascript">
	...
	$(document).on('click', '.sm_checkbox label', function() {
		...
		if(list.indexOf(id) == -1) {
			if(list.length >= 2) {
				alert('Số sản phẩm so sánh đã đạt mức tối đa là 2 sản phẩm.');
				return false;
			}
			list[list.length] = id;
	...
	$(document).on('click', '.li-select-car-tg', function() {
		$('#li-select-car').click();
	});
...
<form id="form-compare" style="display: none;" method="POST" action="<?= Yii::app()->createUrl("car/car/compare/") ?>">
	<input id="list-id-compare" value="<?= implode(',', $selected_car)  ?>" name="id">
</form>
<div id="banner02">
				...
				<div class="content_tab">
					<div class="row">
						...
						<div class="tab-content">
							...
							<p class="compare-mobile li-compare-car-tg">
								<a>So sánh</a>
							</p>
						</div>
					</div>
